...
// Static project details for local projects
const STATIC_PROJECT_DETAILS: Record<string, any> = {
  'proj-eturkey-001': {
    id: 'proj-eturkey-001',
    title: 'eTurkey — Real Estate Platform',
    description:
      'A modern real estate web platform for buying, renting, and investing in properties in Antalya, Turkey. Built with a sleek dark design, the platform features a 3D animated building visualization on the hero section, smooth scroll navigation, and developer-direct property listings. Designed to serve Russian-speaking clients looking for premium real estate in Turkey\'s most prestigious districts.',
    technologies: 'HTML5, CSS3, JavaScript, Three.js, GSAP, Responsive Design',
    key_features:
      '3D interactive building visualization,Property search and filtering,Responsive mobile-first layout,Smooth scroll animations,Developer-direct listings,Multi-language support ready,Consultation booking form,WhatsApp integration',
    image_url: '/assets/eturkey.png',
    image_urls: ['/assets/eturkey.png'],
    live_url: 'https://ruslankyzyljol-ai.github.io/eturkey1/',
    github_url: 'https://github.com/ruslankyzyljol-ai/eturkey1',
  },
}
...
export default function PortfolioDetailPage() {
  ...
  const fetchProject = async () => {
    ...
    if (idStr && STATIC_PROJECT_DETAILS[idStr]) {
      setProject(STATIC_PROJECT_DETAILS[idStr])
      return
    }
  ...
  const nextImage = () => {
    if (currentImage < galleryImages.length - 1) setCurrentImage((p) => p + 1)
  }
  const prevImage = () => {
              ...
              >
                <div className="relative rounded-[26px] overflow-hidden border border-white/10 bg-gradient-to-br from-[#111] to-[#171717] max-w-[560px] mx-auto">
                  <motion.img
                    ...
                    className="w-full h-[220px] md:h-[280px] object-cover cursor-pointer"
                  />
                  {currentImage > 0 && (
                    <button
                      onClick={prevImage}
                    ...
                    {galleryImages.map((_: any, i: number) => (
                      <motion.button
                        ...
                        className={`rounded-full transition-all duration-300 ${
                          currentImage === i ? 'w-6 h-1.5 bg-white' : 'w-1.5 h-1.5 bg-white/30'
                        }`}
                      />
                    ))}
                  </div>
